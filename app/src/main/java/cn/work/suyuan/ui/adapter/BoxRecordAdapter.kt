...
class BoxRecordAdapter :
    BaseQuickAdapter<TokenData.Data.DataArrays, BaseViewHolder>(R.layout.adapter_box_record) {
    override fun convert(holder: BaseViewHolder, item: TokenData.Data.DataArrays) {
        if (fmStatus == 1) setTextTitle(holder,"ID","操作员","箱码","时间","IP")
        else setTextTitle(holder,"ID","外箱条码","内箱条码","产品数量","归属大箱条码")
